...
scan_content() {
    ...
    if [ -n "$extra_exclude" ]; then
        matches=$(git grep -Il -E "$pattern" -- . ':(exclude)Scripts/check_distribution_privacy.sh' "$extra_exclude" || true)
    else
...
scan_content '固定されたGitHubアカウントを検出しました' '(raw\.githubusercontent\.com|github\.com)/[A-Za-z0-9_.-]+/VoiceMovieStudio' ':(exclude)updates/appcast.xml'
